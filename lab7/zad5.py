...
min_mess_num = 0
min_mess_size = float("inf")

for line in response_lines:
    mess_num, mess_size = line.split()
    mess_num = int(mess_num)
    mess_size = int(mess_size)
    if mess_size < min_mess_size:
        min_mess_size = mess_size
        min_mess_num = mess_num    
print(f"the smallest mess {mess_num}, size {mess_size}")

connection.write(b"DELE " + str(min_mess_num) + b"\r\n")
response = connection.read_until(b"\r\n")
